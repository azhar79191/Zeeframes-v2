...
const MissionSection = () => {
  return (
      <section className='z-50 py-20 relative bg-[#0B0B0B]'>
        <div className='container'>
           ...
           <div className='flex justify-between  mt-20'>
                  <p className="text-white text-sm font-['Inter'] uppercase">Since 2020, what we get...</p>
                  <div className="grid grid-cols-2 gap-18 ">
                    {itemsData.map((item,index)=>(
                        <div key={index}>
                            <div className='relative flex flex-col justify-end gap-1 mt-17  z-70  '>
                                <span className='absolute -z-10 right-0 bottom-10'>
                                    {item.icon}
                                </span>
                                <span className="text-[#F3FE00] text-6xl font-bold font-['Inter']">{item.count}</span>
                                <p className="text-[#C2C2C2] text-[20px] font-['Inter']">{item.text}</p>
